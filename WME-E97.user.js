...
(function () {
  ...
  function ready () {
    ...
    new APIHelperUIShortcut(NAME + '-copy', I18n.t(NAME).title, NAME, I18n.t(NAME).title, 'C+D', copy, null).add()
  }

  function inject (event, element) {
    if (element.querySelector('div.external-providers-control > wz-label')) {
      element.querySelector('div.external-providers-control > wz-label').append(button)
    }
  }
